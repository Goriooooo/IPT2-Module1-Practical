...
export const syncReservationToCalendar = async (reservation) => {
  try {
    ...
    // Create a Date object from the reservation date in local timezone
    ...
    const month = reservationDateObj.getMonth() + 1; // getMonth() is 0-indexed
    const day = reservationDateObj.getDate(); // getDate() gives day of month in local time
    
    const [hours, minutes] = reservation.time.split(':').map(Number);
    
    // Format date-time string for Google Calendar (RFC3339)
    const pad = (num) => String(num).padStart(2, '0');
    const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    
    // Create ISO 8601 date-time strings with local date components
    const startDateTime = `${year}-${pad(month)}-${pad(day)}T${pad(hours)}:${pad(minutes)}:00`;
    const endDateTime = `${year}-${pad(month)}-${pad(day)}T${pad(hours + 2)}:${pad(minutes)}:00`;

    console.log('📅 Original reservation date:', reservation.date);
    console.log('📆 Parsed local date:', `${year}-${pad(month)}-${pad(day)}`);
    console.log('⏰ Event start time:', startDateTime);
    console.log('⏰ Event end time:', endDateTime);
