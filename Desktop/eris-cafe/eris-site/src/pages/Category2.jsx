...
import HotEspressoPlaceholder from '../assets/ESPRESSOHOT.png';
...
const Category2 = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const hotEspressoProducts = products.filter(p => 
        p.category === 'Hot Espresso Series' || 
        p.name.includes('HOT') ||
        p.name.includes('(HOT)') ||
        p.category === 'Hot Coffee'
      );
      
      console.log('Filtered hot espresso products:', hotEspressoProducts.length);
      
      if (hotEspressoProducts.length === 0) {
      ...
      const productsWithImages = hotEspressoProducts.map(product => ({
        ...product,
        // Prioritize Cloudinary image from database, fallback to placeholder
        image: product.image || HotEspressoPlaceholder,
        category: product.category || 'Hot Espresso Series',
  ...
  return (
    ...
    >
     {/* Header */}
      ...
      {/* Header */}
      <h1 className='font-playfair text-center mt-12 text-5xl'>Hot Espresso Series</h1>
