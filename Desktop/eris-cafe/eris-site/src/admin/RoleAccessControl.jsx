...
const RoleAccessControl = () => {
    ...
    const fetchRolePermissions = async () => {
      try {
        ...
        if (permissionsResponse.data.success && usersResponse.data.success) {
          ...
          // Sort roles: Admin first, then Owner, then Staff
          const roleOrder = { admin: 1, owner: 2, staff: 3 };
          const sortedData = permissionsResponse.data.data.sort(
  ...
  const getRoleDescription = (role) => {
    const descriptions = {
      ...
      owner: 'Business owner - Access controlled by Admin via RBAC',
  ...
  const handleEditRole = (role) => {
    ...
    if (currentUserRole !== 'admin') {
      Swal.fire({
        title: 'Access Denied',
        text: 'Only Admins can modify role permissions. Owners and Staff permissions are controlled by Admin.',
        icon: 'error',
        confirmButtonColor: '#8B5CF6'
      });
      return;
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-white">
              ...
              <p className="text-sm text-white/90">
                ...
                Admins can restrict <span className="font-semibold">Owner</span> and <span className="font-semibold">Staff</span> access to any module including Settings, RBAC, and Login Monitoring.
