...
import NonCoffeePlaceholder from '../assets/NONCOFFEE.png';
...
const Category3 = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Filter by category on frontend - Only exact category matches, no name-based filtering
      const nonCoffeeProducts = products.filter(p => 
        p.category === 'Non-Coffee' || 
        p.category === 'Non Coffee' ||
        p.category === 'Tea' ||
        p.category === 'Smoothies' ||
        p.category === 'Milkshakes'
      );
      
      console.log('Filtered non-coffee products:', nonCoffeeProducts.length);
      
      if (nonCoffeeProducts.length === 0) {
      ...
      const productsWithImages = nonCoffeeProducts.map(product => ({
        ...product,
        // Prioritize Cloudinary image from database, fallback to placeholder
        image: product.image || NonCoffeePlaceholder,
        category: product.category || 'Non-Coffee',
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <h1 className='font-playfair text-center mt-12 text-5xl'>Non-Coffee Series</h1>
