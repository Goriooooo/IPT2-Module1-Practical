...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    // Enhance cart items with product availability and stock info
    const enhancedCart = user.cart.map(item => {
      const cartItem = item.toObject();
      if (item.productId) {
        cartItem.isAvailable = item.productId.isAvailable !== undefined ? item.productId.isAvailable : true;
        cartItem.stock = item.productId.stock || 0;
        // Keep productId as ID string for frontend
        cartItem.productId = item.productId._id;
      }
      return cartItem;
    });

    console.log(`Cart loaded for user ${req.user.id}: ${enhancedCart.length} items`);
    
    res.json({ success: true, data: enhancedCart });
  } catch (error) {
